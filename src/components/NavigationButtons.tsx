...
export default function NavigationButtons() {
	...
	const links = [
		...
		},
		{
			...
			label: 'View All Class Schedules'
		},
		{
			role: 'Administrator',
	...
	return (
		<div className='grid grid-cols-6 gap-3 my-4 min-max'>
			{links
				.filter(link => link.role === role || link.role === 'All')
				.map((link, index) => (
					<Link
						href={link.path}
						key={index}
						className={`block ${
							pathname === link.path
								? 'bg-abyss hover:bg-abyss/95'
								: 'bg-stone hover:bg-stone/95'
						} text-white font-medium rounded-lg p-3 space-y-1.5`}
					>
						<svg
